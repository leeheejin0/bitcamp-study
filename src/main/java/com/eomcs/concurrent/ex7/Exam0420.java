...
public class Exam0420 {
    ...
    public void run() {
      try {
        ...
        for (long i = 0; i < 10000000; i++) {
          double result = Math.tan(134567.8976)/Math.random();
        }
  ...
  public static void main(String[] args) {
    ...
    //    바로 스레드를 멈출 기회라고 보고 스레드를 종료을 시도할 것이다. 
    //  - 이런 이유로 shutdown() 보다는 스레드 종료를 좀더 강제하는 효과가 있다. 
    // => 작업 중인 스레드가 Not Runnable 상태가 아니라면 작업이 끝날 때까지 기다린다. 
    // => 다만 작업을 마치고 대기 중인 스레드는 즉각 취소한다. 
